...
export default function Matches({ variant = "global", highlightPlayerId }: Props) {
	...
	const loadMore = useCallback(async () => {
		...
		// Set highest game number (latest match)
		if (data.length > 0 && highestGameNumber === null) {
			setHighestGameNumber(matches.length + data.length);
		}

		setMatches((prev) => [...prev, ...data]);
		setHasMore(data.length === PAGE_SIZE);
		setLoading(false);
	}, [matches.length, loading, hasMore, highestGameNumber]);
	...
	function outline(playerId: string) {
		...
		return highlightPlayerId === playerId ? "border-green-500/30" : "border-red-500/30";
	}
