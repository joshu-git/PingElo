...
export default function Matches({
	...
	const [highestGameNumber, setHighestGameNumber] = useState<number | null>(
		null
	);
		...
		async function fetchHighestGameNumber() {
			...
			if (variant === "profile" && highlightPlayerId) {
				query = supabase
					...
					.select("id", { count: "exact", head: true })
					.or(
						`player_a1_id.eq.${highlightPlayerId},player_b2_id.eq.${highlightPlayerId}`
					);
			}
			...
			<div className="w-full max-w-2xl space-y-6">
				...
				<div className="flex flex-col sm:flex-row justify-between items-center gap-4">
					<h1 className="text-2xl md:text-3xl font-bold tracking-tight">
						Matches
						{highestGameNumber ? ` (${highestGameNumber})` : ""}
					</h1>
								...
								<div className="flex flex-col sm:flex-row gap-5 sm:items-center sm:justify-between">
									...
									<div className="flex flex-col gap-3 flex-1 min-w-0">
										...
										>
											...
											<span className="ml-4 text-sm text-text-subtle w-14 text-right">
												{m.elo_change_a > 0 && "+"}
												{m.elo_change_a}
											</span>
										</Link>
